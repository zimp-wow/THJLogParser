...
namespace WinFileIO
{
    ...
    public unsafe class WinFileIO : IDisposable
    {
        ...
        public void OpenForWriting(string FileName, WinFileFlagsAndAttributes flagsAndAttributes)
        {
            ...
            Close(false, true);
            pHandleWrite = CreateFile(FileName, GENERIC_WRITE, 0, UIntPtr.Zero, CREATE_ALWAYS, (uint)flagsAndAttributes, IntPtr.Zero);
            position = 0;
        ...
        */
        ...
        public static void CopyFile(string FromPath, string ToPath)
        {
            ...
            {
                reader.OpenForReading(FromPath);
                writer.OpenForWriting(ToPath);
            ...
            System.IO.FileInfo origFinfo = new System.IO.FileInfo(FromPath);
            System.IO.File.SetLastWriteTime(ToPath, origFinfo.LastWriteTime);
        }
        public static void WriteFile(string ToPath, byte[] data)
        {
            WriteFile(ToPath, data, data.Length);
        }
        public static void WriteFile(string ToPath, byte[] data, int length)
        {
            using (WinFileIO writer = new WinFileIO(data))
            {
                writer.OpenForWriting(ToPath);
                writer.WriteBlocks(length);
            }
        }
        public static void WriteFile(string ToPath, System.IO.MemoryStream data)
        {
            byte[] wfioBlocks = data.ToArray();
            int length = wfioBlocks.Length;
            using (WinFileIO writer = new WinFileIO(wfioBlocks))
            {
                writer.OpenForWriting(ToPath);
                writer.WriteBlocks(length);
        ...
        public static byte[] ReadFile(string FromPath)
        {
            ...
            using (WinFileIO reader = new WinFileIO(wfioBlocks))
            {
                ...
                {
                    int read = reader.ReadBlocks(wfioBlocks.Length);
                    ms.Write(wfioBlocks, 0, read);
                }
            }
            return ms.ToArray();
        }
        public static System.IO.MemoryStream ReadFileStream(string FromPath)
        {
            System.IO.MemoryStream ms = new System.IO.MemoryStream();
            byte[] wfioBlocks = new byte[WinFileIO.BlockSize];
            using (WinFileIO reader = new WinFileIO(wfioBlocks))
